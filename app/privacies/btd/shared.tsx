...
import { Trophy, Sun, Moon, Mail } from 'lucide-react';
...
(function () {
  try {
    var saved = localStorage.getItem('btd-legal-theme');
...
function useTheme() {
  ...
  const toggle = () => {
    ...
    localStorage.setItem('btd-legal-theme', next ? 'dark' : 'light');
    setDark(next);
  };
...
export function BrandLogo() {
  return (
    <Link href={APP.basePath} className="flex items-center gap-2.5">
      ...
      >
        <Trophy className="w-4 h-4" style={{ color: 'var(--accent-on)' }} />
      </span>
