...
export function DownloadCta({ large = false, outline = false }: { large?: boolean; outline?: boolean }) {
  if (outline) {
    return (
      <Link
        ...
        className={inline-flex items-center justify-center gap-2 font-bold rounded-2xl border transition-all }
        style={{
  ...
  return (
    <Link
      ...
      target=_blank
      rel=noopener noreferrer
      className={inline-flex items-center justify-center gap-2.5 font-bold rounded-2xl transition-all shadow-md hover:shadow-lg }
      style={{
        background: 'linear-gradient(135deg, var(--accent), var(--accent-hover))',
        color: 'var(--accent-on)',
