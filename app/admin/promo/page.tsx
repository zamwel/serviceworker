...
export default function PromoAdmin() {
  ...
  async function remove(id: string) {
    ...
    await fetch(`/api/promo/admin/coupons?id=${id}`, {
      method: "DELETE",
      headers: headers(),
    });
    load();
      ...
      <div className="min-h-screen bg-[#0A0A0A] text-white grid place-items-center px-6">
        <div className="w-full max-w-sm bg-white/5 border border-white/10 rounded-3xl p-8">
          ...
          <p className="text-gray-400 text-sm mb-6">
            Enter the admin secret to manage coupons.
          </p>
          <input
            type="password"
            className={input}
            placeholder="PROMO_ADMIN_SECRET"
            value={secret}
          ...
          <button
            onClick={saveSecret}
            className="mt-5 w-full bg-red-600 hover:bg-red-500 font-bold py-3 rounded-xl transition-colors"
          >
            Unlock
          </button>
        </div>
              ...
              <div className="mt-5 bg-black/40 border border-white/10 rounded-2xl p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={copyCodes} className="text-xs text-gray-300 hover:text-white">
                      Copy
                    </button>
                    <button onClick={downloadCsv} className="text-xs text-gray-300 hover:text-white">
                      CSV
                    </button>
                  </div>
                </div>
                    ...
                    {coupons.map((c) => (
                      <tr key={c.id} className="border-b border-white/5">
                        ...
                        <td className="py-2.5 pr-4 text-gray-300">
                          {c.rewardType}
                          {c.rewardType === "subscription" && c.durationDays
                            ? ` (${c.durationDays}d)`
                            : ""}
                        </td>
                        <td className="py-2.5 pr-4 text-gray-300">
                        ...
                        <td className="py-2.5 text-right whitespace-nowrap">
                          ...
                          <button
                            onClick={() => patch(c.id, { active: false, revokeGrants: true })}
